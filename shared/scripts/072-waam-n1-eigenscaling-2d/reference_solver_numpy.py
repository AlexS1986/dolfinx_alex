...
problem as solve_plane_stress_eigen.py (dolfinx). Its only purpose is to
cross-validate the dolfinx implementation (same mesh, BCs, material law -
`materials_eigen_2d.py` is shared). All reported results come from dolfinx.

To keep the two apart, everything written here lands in `verification/`.
...
Usage: python3 reference_solver_numpy.py --micro <path>/micro_roi.npz --tag base
"""
...
def solve(micro, cfg, afuns, eps0=1e-3, rtol=1e-10, verbose=True):
    d = np.load(micro)
    ...
    Cc, Exc, a_maps, minfo = M2.build_cell_tensors(
        euler, phase, gid, zone, (np.arange(nx)+0.5)*h, cfg, afuns,
        verbose=verbose)
    Cfe = Cc[::-1]                                  # FE row-major bottom-up
    zfe = zone[::-1]; pfe = phase[::-1]
    Exfe = Exc[::-1]; gfe = gid[::-1]
    afe = {n: a_maps[n][::-1] for n in M2.FACTOR_NAMES}
    ...
    fields = {'u': u.reshape(nny, nnx, 2), 'sig': sigE.reshape(ny, nx, 3),
              ...
              'Ex_fe': Exfe, 'grain_fe': gfe,
              **{f'{n}_fe': afe[n] for n in M2.FACTOR_NAMES}}
    return out, fields
...
def main():
    ...
    ap.add_argument('--sfun', default=None, help='overrides all three factors')
    ...
    micro = args.micro if os.path.isabs(args.micro) else os.path.join(here, args.micro)
    out, fields = solve(micro, cfg, afuns, eps0=args.strain)
    out['tag'] = args.tag
    out['factors'] = ({n: args.sfun for n in M2.FACTOR_NAMES}
                      if args.sfun is not None
                      else {'aK': args.aK, 'aCp': args.aCp, 'aC44': args.aC44})
    out['solver'] = 'numpy-Q1-reference (VERIFICATION ONLY, not a result)'
    print(json.dumps({k: v for k, v in out.items() if k != 'E_profile'}, indent=2))
