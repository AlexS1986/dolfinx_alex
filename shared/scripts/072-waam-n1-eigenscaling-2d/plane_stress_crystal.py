...
# ==========================================================================
# 072 additions: eigen-decomposition of the CUBIC stiffness into its three
# irreducible parts (bulk / tetragonal shear / trigonal shear) so each part
# can be scaled SEPARATELY (project 072-waam-n1-eigenscaling-2d).
#
#   K   = (C11 + 2*C12) / 3      bulk modulus            (eigenvalue 3K)
#   Cp  = (C11 - C12) / 2        tetragonal shear C'     (eigenvalue 2C')
#   C44                          trigonal shear          (eigenvalue 2C44)
#
# Zener anisotropy A = C44 / Cp.  Scaling the three parts with factors
# (aK, aCp, aC44) keeps cubic symmetry; aK=aCp=aC44=s reproduces the scalar
# prefactor s of project 070 exactly.  Positive definiteness <=> all three
# scaled moduli > 0, i.e. any positive factors are admissible.
# ==========================================================================

def cubic_to_KCpC44(C11, C12, C44):
    """Cubic constants -> irreducible moduli (K, C', C44)."""
    return (C11 + 2.0 * C12) / 3.0, (C11 - C12) / 2.0, C44


def cubic_from_KCpC44(K, Cp, C44):
    """Irreducible moduli (K, C', C44) -> cubic constants (C11, C12, C44)."""
    return K + 4.0 * Cp / 3.0, K - 2.0 * Cp / 3.0, C44


def cubic_eigen_parts(C11, C12, C44):
    """Split the cubic 6x6 stiffness into its three irreducible parts
    (each a 6x6 in the crystal frame) so that for any factors

        C_scaled = aK * Ch + aCp * Ct + aC44 * Cs

    equals cubic_C(*cubic_from_KCpC44(aK*K, aCp*Cp, aC44*C44)).
    Rotation is linear, so the parts may be Bond-rotated individually and
    recombined afterwards (used for per-grain caching)."""
    K, Cp, C44 = cubic_to_KCpC44(C11, C12, C44)
    Ch = cubic_C(K, K, 0.0)                              # hydrostatic
    Ct = cubic_C(4.0 * Cp / 3.0, -2.0 * Cp / 3.0, 0.0)   # tetragonal shear
    Cs = cubic_C(0.0, 0.0, C44)                          # trigonal shear
    return Ch, Ct, Cs


def cubic_scaled(C11, C12, C44, aK=1.0, aCp=1.0, aC44=1.0):
    """Cubic 6x6 with the three irreducible parts scaled by (aK, aCp, aC44)."""
    K, Cp, C44_ = cubic_to_KCpC44(C11, C12, C44)
    return cubic_C(*cubic_from_KCpC44(aK * K, aCp * Cp, aC44 * C44_))
