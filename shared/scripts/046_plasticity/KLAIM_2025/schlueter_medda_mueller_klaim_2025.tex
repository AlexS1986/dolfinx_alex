...
\title[Influence of Material Constitutive Laws on Effective Crack Resistance in Heterogeneous Materials]{Influence of Material Constitutive Laws on Effective Crack Resistance in Heterogeneous Materials}
...
The objective of this approach is the determination of an effective energetic fracture toughness $J_c^{\text{eff}}$, which can subsequently be employed as a material parameter for predicting macroscopic crack growth according to the fracture criterion~(\ref{eq:fracture_criterion_J}).

The numerical experiment is structured as follows.
Instead of performing a physical experiment, a representative region\footnote{The term representative volume element is 
...
separation of length scales.}
 is selected that explicitly resolves the relevant microstructural features and whose fracture behavior is to be captured by $J_c^{\text{eff}}$.
...
The scale at which $J_c^{\text{eff}}$ is applied is denoted as the \textsl{macroscale}.
...
The strain energy density is given by $\psi_{\text{el}}$, the linearized strain tensor\footnote{In this work, only small strain assumptions are considered. However, the framework could be extended to finite strains
...
Independence from the initial microscopic crack configuration and adequate representation of fracture mechanisms can be ensured by selecting a sufficiently large representative region that contains an appropriate number of microstructural features\footnote{This presumes a sufficiently homogeneous spatial distribution of microstructural features, analogous to assumptions made for representative volume elements in micromechanics.}.
...
Details of this model are e.g. described in ~\cite{kuhn_continuum_2010}. The governing equations 
...
Simulations were conducted for the three parameter sets introduced in Section~\ref{sec:parameter_sets}. To intentionally trigger crack nucleation at $x = 0$, the fracture toughness $J_c$ was locally reduced by 1\% in the region $-3\Delta h \le x \le 3\Delta h$.
...
\end{equation}
Employing Voigt notation indicated by underscores $\underline{(*)}$, the 2D train tensor
\[
...
An initial crack is prescribed in the plane $y=0$ for $x-x_{\rm min}\le w_c$, where $x_{\rm min}$ denotes the minimum $x$-coordinate of the mesh. Along the expected crack path, the finite element edge length is set to $\Delta h = 0.02L$.
...
Simulations are conducted for wall widths $w_s\in\left[0.25L,4.0L\right]$ using the parameter set \vMLabel.

Figures~\ref{fig:one_row_two_cols}a) and b) illustrate the phase-field variable $s$ and the plastic variable $\alpha$, respectively, for $w_s=0.25L$. The crack propagates through the central row of pores and is able to grow continuously between pores, in contrast to the two-dimensional case. Consequently, the crack tip position $x_{ct}$\footnote{The microscopic crack tip postion $x_{ct}$ is indentified as the node with the largest $x$-coordinate that fulfills $s=0$.} evolves more smoothly, as confirmed by Fig.~\ref{fig:position_3D}.

The J-integral is computed according to (\ref{eq:J-integral}) and normalized by the domain width in the $z$-direction,
\begin{equation}
...
numerical fracture resistance of the von Mises-type material law $J_x^{\text{num,pl}}\approx1.50J_c^0$, c.f. section~\ref{sec:steady_growth_griffith}.
