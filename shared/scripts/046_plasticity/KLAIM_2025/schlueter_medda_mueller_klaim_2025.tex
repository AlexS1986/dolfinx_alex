...
% Default build: clean output with all changes accepted.
% Marked-up build: compile schlueter_medda_mueller_klaim_2025_changes.tex.
\ifdefined\ShowChanges
\usepackage[markup=underlined,authormarkup=none]{changes}
\else
\usepackage{xparse}
\NewDocumentCommand{\added}{O{} m}{#2}
\NewDocumentCommand{\deleted}{O{} m}{}
\NewDocumentCommand{\replaced}{O{} m m}{#3}
\NewDocumentCommand{\highlight}{O{} m}{#2}
\NewDocumentCommand{\definechangesauthor}{O{} m}{}
\fi
...
\title[Influence of J2-plasticity on the effective crack resistance of a simplified porous material in a numerical experiment]{\deleted{Influence of Material Constitutive Laws on Effective Crack Resistance in Heterogeneous Materials}\added{Influence of J2-plasticity on the effective crack resistance of a simplified porous material in a numerical experiment}}
...
\abstract{  
      In this work, we investigate the simulation-based determination of the \deleted{effective fracture
resistance}\added{effective crack resistance} of porous materials while accounting for nonlinear, ductile material behavior.
Following the approach of Hossain et al.~\cite{hossain_effective_2014}, numerical experiments
with surfing boundary conditions are conducted to identify the \deleted{effective crack resistance as
a macroscopic material parameter}\added{effective crack resistance}. Crack evolution in the microstructure is modeled using a
...
macroscopic crack advance and is taken as the \deleted{effective fracture resistance}\added{effective crack resistance}. In contrast to
previous studies, a von~Mises plasticity model is employed to investigate ductile fracture in
porous metals. The results show that the \deleted{effective fracture resistance}\added{effective crack resistance} is governed primarily
...
model confirms these findings, showing similar saturation behavior of the \deleted{effective fracture
resistance}\added{effective crack resistance} with respect to microstructural wall width and good quantitative agreement with
...
as demonstrated by Hauser et al.~\cite{hauser_porosity_2021}. Fracture-mechanics-based criteria are often the appropriate tools
...
of active debate, as discussed by Lebihain~\cite{lebihain_brittle_2021},
...
Ernesti and Schneider~\cite{ernesti_fast_2021,ernesti_computing_2022} for anisotropic fracture resistance.

Michel and Suquet~\cite{michel_merits_2022} \deleted{propose the effective fracture
...
a representative material region containing
 ...
 to so-called surfing boundary conditions that enforce complete separation, and the energetic driving force required for this process is recorded.}
  \added{A recent related development is the variable stiffness boundary condition of Hao et al.}~\cite{hao_variable_2026}\added{, which builds on the surfing boundary condition but translates a simple uniform remote displacement into a non-uniform load through an engineered stiffness gradient, thereby facilitating implementation in experiments and commercial finite-element software.}
The resulting microstructural crack evolution is
then simulated using \deleted{such a}\added{a} phase-field model.
See, for example, the review by Ambati et al.~\cite{ambati_review_2015} for a comprehensive overview\added{ of phase-field models for fracture}.
\added{For ductile materials, phase-field fracture has also been coupled to plasticity in variational finite-strain formulations, for example by Miehe, Aldakheel and co-workers}~\cite{miehe_phasefield_ductile_2016,miehe_phasefield_porous_2016}\added{.}

\deleted{Similarly, in }In previous studies by the authors of this contribution~\cite{schlueter_ermittlung_2025,schluter_determination_2025},
\deleted{this framework}\added{the aforementioned framework} was employed to determine the \deleted{effective fracture toughness}\added{effective crack resistance} of \deleted{porous materials}\added{a simplified porous material},
assuming linear-elastic bulk behavior. \deleted{For a simplified porous material, it}\added{It} was demonstrated that
the renucleation mechanism of cracks that are trapped in pores
determines the \deleted{effective fracture toughness}\added{effective crack resistance}
and can act as a toughening factor if the pores are
spaced far enough apart. A saturation of the \deleted{effective toughness}\added{effective crack resistance} was obtained above
...
\added{These observations should be understood as results for the specific porous material and numerical experiment investigated in those studies, not as a general statement for arbitrary porous materials.}
...
\deleted{The results indicate that the energy dissipation capacity of the material law,
...
\added{For the specific idealized porous material and the crack-growth experiment following Hossain et al.,
...
of the \deleted{effective fracture toughness}\added{effective crack resistance} for a simplified porous material
...
\section{A Numerical Experiment to Determine the \deleted{Effective Toughness}\added{Effective Crack Resistance}
...
The numerical experiment proposed by Hossain et al.~\cite{hossain_effective_2014} for the determination of an \deleted{effective fracture toughness}\added{effective crack resistance} is conceptually closer to physical experiments than to the classical homogenization concepts commonly presented in micromechanics textbooks such as those by Gross and Seelig~\cite{gross_fracture_2011} and Nemat-Nasser and Hori~\cite{nemat-nasser_micromechanics_2013}. 
\added{It should therefore be understood as an operational definition of an effective crack resistance for a prescribed numerical experiment, not as a classical homogenization result.}
The objective of this approach is the determination of an \deleted{effective energetic fracture toughness}\added{effective energetic crack resistance}\deleted{ $J_c^{\text{eff}}$, which can subsequently be employed as a material parameter for predicting macroscopic crack growth according to the fracture criterion~(\ref{eq:fracture_criterion_J}).}\added{, denoted by $J_{x,c}^{\text{eff}}$. This quantity can subsequently be used as an input quantity in a macroscopic fracture criterion such as~(\ref{eq:fracture_criterion_J}).}
\added{Such a use is justified only to the extent that the fracture process analyzed by~(\ref{eq:fracture_criterion_J}) is representative of the specific material and loading situation considered in the numerical experiment.}
...
separation of length scales.}
 is selected that explicitly resolves the relevant microstructural features and whose fracture behavior is to be captured by \deleted{$J_c^{\text{eff}}$}\added{the effective energetic crack resistance}.
...
The scale at which \deleted{$J_c^{\text{eff}}$}\added{this effective energetic crack resistance} is applied is denoted as the \textsl{macroscale}.
...
The union of the representative region and the surrounding matrix is considered as a continuum~$\Omega$ \added{with boundary $\partial\Omega$}. 
...
Displacements associated with mode~I crack opening are prescribed at the boundary \added{$\partial\Omega$} using a fictitious 
...
\added{Here, $t$ denotes the pseudo-time, or loading parameter, used to advance the fictitious crack tip.}
...
\begin{equation}
    ...
    \label{eq:J-integral}
\end{equation}
where $\boldsymbol{e}_x$ denotes the unit vector in the macroscopic crack propagation direction\added{; hence, the index $x$ indicates both the crack-growth direction and the direction onto which the J-Integral is projected.}
\deleted{The strain energy density is given by $\psi_{\text{el}}$}\added{The strain energy density entering the J-Integral is denoted by $\psi_{\text{el}}$ and is specified in Section~\ref{sec:phase_fiel_model_ductile}}, the linearized strain tensor\deleted{In this work, only small strain assumptions are considered. However, the framework could be extended to finite strains
...
\added{The recorded value is, of course, affected by the crack pattern inside the explicitly resolved microstructure; however, following the observation reported by Hossain et al.~\cite{hossain_effective_2014}, the additional homogeneous surrounding material is assumed not to change the value of the boundary J-Integral, so that evaluating it at the boundary of the resolved microstructure or at the boundary of an embedding region with representative effective properties gives the same macroscopic driving force.}

The \deleted{effective fracture toughness}\added{effective crack resistance}, denoted by \deleted{$J_c^{\text{eff}}$}\added{$J_{x,c}^{\text{eff}}$}, is defined by Hossain et al.~\cite{hossain_effective_2014} as the maximum value attained by the J-Integral during the complete separation process\added{, i.e., during characteristic crack growth through the representative region},
\begin{equation}
    J_{x,c}^{\text{eff}} = \max_t J_x(t).
\end{equation}
\added{The index $c$ denotes the critical crack-resistance value obtained from this maximum, whereas the index $x$ is retained to avoid implying isotropic crack resistance.}
...
\added{This definition should not be confused with a proof that $J_{x,c}^{\text{eff}}$ converges to a material property under enlargement of the representative region. Establishing such convergence, if it exists for a given class of microstructures and loading paths, would require a separate study.}

\added{For the numerical experiment introduced by Hossain et al.}~\cite{hossain_effective_2014}\added{, two formulations of the surfing boundary conditions were compared in}~\cite{hossain_effective_2014, bohnen_simulation_2024}\added{. For the specific cases investigated there, including the formulation used in the present work, no relevant difference in the predicted value of $J_{x,c}^{\text{eff}}$ was found. This should not be understood as a general proof of independence from the boundary conditions.}
...
\added{In the present work, this requirement is not fully verified: only one prescribed macroscopic crack-growth direction and one initial crack location are considered, and no convergence study with respect to the size of the resolved porous region is performed. The results should therefore be read as a controlled numerical experiment on the influence of J2-plasticity for the selected simplified microstructure, rather than as the determination of a generally valid homogenized fracture toughness.}


\section{\deleted{A phase field model of ductile fracture}\added{Simulation of fracture with phase field models}}\label{sec:simulation_phase_field_models}

\added{This section introduces the phase-field models and numerical ingredients used in the subsequent numerical experiments.}

\subsection{\added{A phase field model of ductile fracture}}\label{sec:phase_fiel_model_ductile}
...
\added{Related variational finite-strain phase-field formulations for ductile fracture and porous plasticity were developed by Miehe, Aldakheel and co-workers}~\cite{miehe_phasefield_ductile_2016,miehe_phasefield_porous_2016,miehe_phasefield_implementation_2017}\added{.}
...
\added{The notation and numerical treatment of the underlying small-strain $J_2$ plasticity model follow standard computational plasticity concepts; see, for example, Simo and Hughes}~\cite{simo_computational_1998}\added{.}
...
To model linear isotropic strain hardening, a scalar internal state variable $\alpha$ is introduced\deleted{ that is related to the density of dislocations in the crystallographic microstructure}. 
...
\added{Irreversibility of crack growth is enforced in both the ductile and the linear elastic phase-field formulations by fixing the phase field at fully fractured points: if $s(\boldsymbol{x},t_{\boldsymbol{x}})=0$ for the first time $t_{\boldsymbol{x}}$ at a point $\boldsymbol{x}$, then a Dirichlet condition $s(\boldsymbol{x},t)=0$ is imposed for all later pseudo-times $t>t_{\boldsymbol{x}}$.}
...
\begin{equation}
    ...
    \label{eq:deg_grad}
\end{equation}
with $\beta = 0.2$ controlling the slope of $g(s)$ at $s=1$, and $\eta = 10^{-5}$ serving as a residual stiffness parameter to avoid ill-conditioned tangents \deleted{when}\added{if} the problem is solved using a Newton scheme.
\added{The cubic degradation function is used because it allows the prescribed yield stress and hardening parameter to be implemented in the phase-field formulation in a direct manner.}
Kuhn et al.~\cite{kuhn_phase_2016} \added{discuss that, for the standard quadratic degradation function, these parameters require a reinterpretation in order to obtain effective material properties that can be related to measured data.}
...
\deleted{Finally, the fracture energy contribution is given by}
\added{Finally, the fracture energy density per unit volume is given by}
\begin{equation}
    \psi_{\rm{fr}} = J_c
    \underbrace{\left(
...
\deleted{Here, $\bar{A}$ approximates the crack surface, $J_c$ denotes the material-specific fracture energy per unit crack surface area, and $\epsilon$ controls the width of the diffuse crack zone.}
\added{Here, $\bar{A}$ approximates the crack surface upon integration over a volume, $J_c$ denotes the material-specific fracture energy per unit crack surface area, and $\epsilon$ controls the width of the diffuse crack zone.}

\deleted{For the linear elastic case, it can be shown that the zero set of $s$ minimizing the total energy converges to minimizers obtained with a sharp crack representation as $\epsilon \to 0$, see~\cite{chambolle_approximation_2004}.}
\added{For the linear elastic case, Ambrosio--Tortorelli-type regularizations approximate the sharp-crack variational formulation in the sense of $\Gamma$-convergence as $\epsilon \to 0$. In this sense, the regularized fields $(\boldsymbol{u},s)$ approximate the displacement field and crack set of the corresponding sharp-crack formulation, while the diffuse crack region represented by low values of $s$ approaches the crack set; no pointwise convergence statement for $s$ is intended here, see Kuhn~\cite{kuhn_numerical_2013} and Chambolle~\cite{chambolle_approximation_2004}.}
...
In order to judge the influence of ductile material behavior on the \deleted{effective fracture toughness}\added{effective crack resistance}, 
...
\added{The weak forms of the coupled boundary-value problems are implemented in FEniCSx using \texttt{dolfinx} version 0.7.4}~\cite{fenicsx_documentation}\added{. The displacement field, the phase field, and, where applicable, the plastic internal variables are solved in a monolithic nonlinear system. The nonlinear residual and its Gateaux derivative are assembled and solved with the PETSc-based Newton solver provided by FEniCSx. The linear subproblems arising within Newton's method are solved with the default PETSc Krylov solver GMRES and the default PETSc ILU preconditioner used by this setup.}

\added{The evolution equation for the phase field, Eq.~(\ref{eq:evolution_s}), is discretized in pseudo-time by an implicit backward Euler scheme. The time step is adapted automatically during the simulation. The maximum number of Newton iterations per time step is set to eight. If a step does not converge within this limit, the time step is reduced by a factor of two and the step is repeated. If convergence is reached in fewer than four Newton iterations, the next time step is increased by a factor of two, subject to the prescribed upper bound for the respective simulation.}

\added{The spatial discretization uses first-order Lagrange finite elements on triangular meshes in two dimensions and tetrahedral meshes in three dimensions. Along the expected crack path, the mesh is locally refined such that the element edge length satisfies $\Delta h \leq \epsilon/2$ and is usually chosen much smaller; see the respective sections for the specific values. The specific values of $\Delta h$ and $\epsilon$ used in the individual simulations are stated together with the corresponding numerical setups below.}
...
as well as on the \deleted{effective fracture resistance}\added{effective crack resistance} of a porous material, is investigated.
...
\added{The parameter set \EqJcLabel is chosen to have the same prescribed microscopic fracture resistance $J_c$ as \vMLabel, whereas \EqSigmaStarLabel is chosen such that the one-dimensional tensile strength is close to that of \vMLabel.}
...
\begin{figure}[htb]
    ...
    \vspace{0.2cm}
    \caption{%
        Deformed contour plot of the phase field $s$ in a strip of size $16L \times 2L$ for the simulation \EqJcLabel. The deformation is scaled by a factor of 0.3. 
    }
...
\section{\deleted{Effective Toughness}\added{Effective Crack Resistance} of a Simplified \deleted{2D}\added{Two-Dimensional} Porous Material}
\label{sec:2Dresults}

\subsection{\added{Equivalent Elastic Medium and Simulation Domain}}
...
\begin{figure}[htb]
    ...
    \caption{%
        ...
        \added{The horizontal axis denotes the pseudo-time $t$ normalized by the time scale $L/\dot{x}_{\mathrm{bc}}$.}
    }
...
\begin{figure}[htb]
    ...
    \begin{minipage}{0.49\textwidth}
        ...
        \caption{Section of the J-Integral $J_x$ vs.\ crack tip position $x_{ct}$ between pores two and four for the parameter set \EqJcLabel and varying $w_s$.}
    ...
    \begin{minipage}{0.49\textwidth}
        ...
        \caption{Section of the J-Integral $J_x$ vs.\ crack tip position $x_{ct}$ between pores two and four for the parameter set \EqSigmaStarLabel and varying $w_s$.}
...
\added{Here, $k$ denotes the load case, i.e., the prescribed macroscopic strain for which the linear displacement boundary condition is applied; several load cases are required to determine the full stiffness tensor.}
...
\end{equation}
Employing Voigt notation indicated by underscores $\underline{(*)}$, the \deleted{2D train}\added{two-dimensional strain} tensor
\[
\boldsymbol{\varepsilon} = 
\begin{bmatrix} \varepsilon_{xx} & \varepsilon_{xy} \\ \varepsilon_{xy} & \varepsilon_{yy} \end{bmatrix}
\]
is represented as the \deleted{3-component}\added{three-component} vector
\begin{equation}
\underline{\boldsymbol{\varepsilon}} = 
\begin{bmatrix} \varepsilon_{xx} \\ \varepsilon_{yy} \\ 2 \varepsilon_{xy} \end{bmatrix},
\end{equation}
where the factor of \deleted{2}\added{two} accounts for engineering shear strain. Similarly, the stress tensor
\[
\boldsymbol{\sigma} = 
...
The $6 \times 3$ porous domain is embedded into a rectangular domain of effective stiffness (blue) with dimensions $(8 w_c) \times 20 L$. \added{The two-dimensional simulations in this section are carried out under plane strain assumptions.} The parameters $\sigma_y$ and $H$ entering the nonlinear part of the material law~(\ref{eq:sigma_plast}) are not homogenized. 
...
\subsection{\added{Fracture Behavior and Effective Crack Resistance}}
...
It is observed that the linear elastic simulation with equivalent $J_c$ (\EqJcLabel) exhibits significantly higher values of $J_x$ than the von~Mises model \vMLabel with the same microscopic $J_c$, while the latter still yields higher $J_x$ values than the linear elastic simulation with equivalent fracture stress \EqSigmaStarLabel. \added{For this fixed wall width, the ordering can be interpreted through the different resistance against crack re-nucleation and the additional energy dissipated during ligament failure; in particular, once the one-dimensional tensile strength is largely eliminated as the dominant difference between models, the enhanced dissipation appears to play an important role. The following parameter study shows that this ordering is representative of the main trend, while the detailed pore interaction length additionally depends on $w_s$.}

\deleted{For $w_s = 1.0L$}\added{For a wall width of $1.0L$}, neither the equivalent one-dimensional fracture stress of the bulk material nor the microscopic energetic fracture resistance $J_c$ provides a reliable indicator of the \deleted{effective fracture toughness}\added{effective crack resistance}. Instead, the total dissipated energy $\Pi^*$ observed in 
the one-dimensional crack nucleation experiment correlates more closely with the $J_x$ response, correctly predicting that the \deleted{effective fracture toughness}\added{effective crack resistance} of \vMLabel lies between those of \EqJcLabel and \EqSigmaStarLabel\deleted{, c.f. also}\added{; see also} Fig.~\ref{fig:energy_vs_displacement}.

The periodically repeating pattern of $J_x$, for example between pores two and four, is characteristic of the microstructure. This pattern is shown for varying $w_s$ in Fig.~\ref{fig:PAPER_04_Jx_vs_xct_all_von_Mises} for \vMLabel, Fig.~\ref{fig:PAPER_03A_Jx_vs_xct_all_linear_elastic_gc10} for \EqJcLabel, and Fig.~\ref{fig:PAPER_03_Jx_vs_xct_all_linear_elastic_gc05679.png} for \EqSigmaStarLabel. Independent of the parameter set, $J_{x,c}^{\text{eff}}$ increases with increasing wall width up to a saturation value. This trend is summarized in Fig.~\ref{fig:PAPER_06b_Jx_vs_wsteg_three_datasets}, which shows the effective macroscopic crack resistance $J_{x,c}^{\text{eff}}$ versus $w_s$ for all parameter sets. Beyond a critical wall width, $J_{x,c}^{\text{eff}}$ no longer increases, indicating saturation. The saturation values are approximately $1.80 J_c^{\text{num}}$ for \vMLabel, $1.9 J_c^{\text{num}}$ for \EqJcLabel, and $1.12 J_c^{\text{num}}$ for \EqSigmaStarLabel. Thus, a pronounced toughening effect is observed \deleted{for $w_s \ge 2L$ in all cases}\added{in all cases for wall widths of at least $2L$}.

The accumulated work of internal forces until fracture, $\Pi^*$, is again found to be the most reliable indicator of high \deleted{effective fracture toughness}\added{effective crack resistance}, particularly in the saturation regime at larger wall widths. In contrast, equivalent one-dimensional tensile strength or equivalent microscopic fracture energy $J_c$ does not consistently predict \deleted{effective fracture toughness}\added{effective crack resistance}. \deleted{For small wall widths $w_s \le 0.6L$}\added{For wall widths up to $0.6L$}, the ductile material law leads to a reduction in \deleted{effective fracture resistance}\added{effective crack resistance} below the values obtained for \EqSigmaStarLabel.

Finally, Fig.~\ref{fig:PAPER_06b_Jx_vs_wsteg_three_datasets} shows that saturation occurs at significantly larger wall widths for \vMLabel than for the linear elastic parameter sets. Since $J_{x,c}^{\text{eff}}$ is determined by the peak value reached immediately prior to re-nucleation, saturation implies that re-nucleation is no longer influenced by neighboring pores. The delayed saturation observed for the ductile material law indicates that plasticity enlarges the interaction zone over which neighboring pores affect re-nucleation, due to the formation of extended plastic deformation zones prior to crack nucleation.
\added{The comparison of \vMLabel and \EqSigmaStarLabel in Fig.~\ref{fig:PAPER_06b_Jx_vs_wsteg_three_datasets} shows that the ductile material law leads to a substantially higher effective crack resistance even though the one-dimensional tensile strengths are almost identical.}
...
\section{\deleted{Effective Toughness}\added{Effective Crack Resistance} of a Simplified \deleted{3D}\added{Three-Dimensional} Porous Material}
\label{sec:3D_results}

\subsection{\added{Three-Dimensional Model and Equivalent Elastic Medium}}
...
\begin{figure}[htb]
        ...
        \begin{picture}(0,0)
                    ...
                    \put(-170,87){$z$}
        \end{picture}
        \caption{\deleted{3D}\added{Three-dimensional} mesh for $w_s=0.25L$.}
        \label{fig:mesh3D}
\end{figure}
...
A $6\times3\times2$ array of these unit cells is assembled to represent a simplified porous material. This array is centered about the plane $y=0$ and embedded into a surrounding cuboidal domain of effective stiffness with dimensions $(8 w_c)\times 20L \times (4 w_c)$. \added{Thus, the unit cell used to determine the effective stiffness is cubic, whereas the surrounding homogeneous domain is a larger cuboidal embedding region assigned this effective stiffness.} The mesh, consisting of bilinear tetrahedral finite elements, is locally refined around the expected fracture plane $y=0$, as shown in Fig.~\ref{fig:mesh3D}.
...
$z=z_{\rm max}$ to mimic plane strain while still accounting for the \deleted{3D}\added{three-dimensional} character of the geometry of the microstructure.
...
\subsection{\added{Fracture Behavior and Comparison with the Two-Dimensional Model}}
...
Figure~\ref{fig:PAPER_VM_Jx_all_3D} compares the evolution of $J_x$ with respect to $x_{ct}$ for the two- and three-dimensional models at $w_s=1.0L$. While the three-dimensional response is significantly smoother, a characteristic periodic pattern with pronounced peaks remains observable. \added{The smoother behavior is consistent with the crack-front evolution shown in Fig.~\ref{fig:position_3D}: in contrast to the two-dimensional case in Fig.~\ref{fig:PAPER_00_xct_pf_vs_xct_KI_diamond}, the crack front can propagate between pores and is not trapped completely by a pore.} These peaks correspond to phases of unsteady crack propagation induced by the pores and define the \deleted{effective fracture toughness}\added{effective crack resistance}. \added{Nevertheless, Figs.~\ref{fig:position_3D} and~\ref{fig:PAPER_VM_Jx_all_3D} still show phases of nearly instantaneous crack advance and elevated values of the J-Integral when the crack front is forced to pass between pores. The comparison in Fig.~\ref{fig:PAPER_06b_Jx_vs_wsteg_2D_3D} therefore indicates that this mechanism still produces a toughening effect of comparable magnitude to the two-dimensional case.}

The periodic pattern between pores one and three
% two and four
 is considered representative of the microstructure and is shown for varying $w_s$ in Fig.~\ref{fig:test12q2_3D}. As in the two-dimensional case, $J_{x,c}^{\text{eff}}$ increases with increasing wall width until saturation is reached.

This trend is summarized in Fig.~\ref{fig:PAPER_06b_Jx_vs_wsteg_2D_3D}, where the \deleted{effective fracture toughness}\added{effective crack resistance} $J_{x,c}^{\text{eff}}$ is plotted as a function of $w_s$ for both the two- and three-dimensional models. The three-dimensional model exhibits a similar qualitative behavior, with a slightly reduced saturation value. This value is about $1.15J_c^{\text{num,pl}}$ in the three-dimensional model, whereas it is about $1.25J_c^{\text{num,pl}}$ in the \deleted{2D}\added{two-dimensional} model. The three-dimensional response also shows a weaker dependence on $w_s$, 
...
In this work, we investigated the \deleted{effective fracture toughness}\added{effective crack resistance} of porous materials under the
...
process governing the \deleted{effective fracture resistance}\added{effective crack resistance} \deleted{of the material}\added{of the simplified porous material considered in these numerical experiments}. Our results indicate that the
energy dissipation capacity of the material law during this renucleation process plays a more
critical role for the \deleted{effective fracture resistance}\added{effective crack resistance} than the tensile strength or the microscopic
energetic fracture resistance. Material models that allow for greater energy dissipation during
renucleation result in higher \deleted{effective toughness}\added{effective crack resistance}, underscoring the importance of considering
nonlinear constitutive effects when designing porous or additively manufactured materials.
\added{The comparison shows that matching this stress level alone does not reproduce the crack resistance obtained with the ductile material law, because the additional energy dissipated during plastic deformation and crack re-nucleation is decisive.}
...
results. Although crack fronts are not fully trapped in pores in the \deleted{3D}\added{three-dimensional} case, the pores still induce
phases of stress-driven instantaneous crack growth and a corresponding peak in the J-integral that
defines the \deleted{effective fracture toughness}\added{effective crack resistance}. Qualitatively, the same saturation behavior of the
\deleted{effective fracture toughness}\added{effective crack resistance} with respect to the wall width is observable in the \deleted{3D}\added{three-dimensional} case. However,
the dependence on the wall width is less pronounced. In addition, a good quantitative agreement
between the \deleted{2D and 3D}\added{two- and three-dimensional} predictions for the \deleted{effective fracture toughness}\added{effective crack resistance} is observed.
...
verification in which the numerically determined \deleted{effective fracture toughness}\added{effective crack resistance} is compared to 
...
\added{A further essential step is to investigate representativity more systematically, including variations of the initial crack position, crack-growth direction, microstructure realization, and domain size. Such studies are required before the operational effective crack resistance used here can be assessed as a material property in a stronger homogenization sense.}
\added{A more detailed separation of the individual mechanisms, in particular stress-driven crack nucleation under plasticity, plastic dissipation during ligament failure, and subsequent crack growth between pores, should also be addressed in future work.}
