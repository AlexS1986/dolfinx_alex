...
transition-region microstructure (dolfinx v0.7.3).

Mesh: regular quadrilateral grid, one cell per microstructure grid cell
(micro_<tag>.npz from preprocess_ebsd_to_grid.py). Per cell, the cubic
single-crystal stiffness of its phase (FCC -> 316L austenite, BCC -> 17-4PH
martensite; config.json) is rotated by the cell's Bunge angles (Bond 6x6),
statically condensed to plane stress (3x3, Voigt [xx,yy,xy], engineering
shear), and - inside the transition zone (zone==1) - multiplied by the scalar
prefactor s(x) (--sfun, default "1.0"; any numpy expression in x [um]).

Loading (x = horizontal map axis = load axis):
    u_x = 0      on x = 0        u_x = eps0*Lx  on x = Lx
    u_y = 0      at corner node (0,0)   (rigid body)
    top/bottom (y = 0, Ly): traction-free.

Outputs: E_<tag>.json (global + per-zone apparent moduli + the constants used),
         fields_<tag>.npz (all cell fields reshaped onto the (ny,nx) grid,
                           consumed by make_figures.py),
         ps_<tag>.xdmf with cell fields
             grain_id        - the grain each cell belongs to (grain structure!)
             E_x_local_GPa   - Young's modulus along x of that grain's own
                               rotated+condensed tensor (varies grain by grain)
             region          - 0=17-4PH, 1=transition, 2=316L
             phase_fcc1_bcc2 - crystal system only (2 values, bookkeeping)
             s_prefactor     - the applied s(x)
             u, sig_xx/yy/xy, eps_xx

Frame handling: the npz stores Bunge angles in the TSL MAP frame (y down);
this script builds the mesh y-UP and converts orientations with a 180deg
rotation about x (plane_stress_crystal.FLIP_X180). Row 0 of the npz = top of
the map = y_fe = Ly.

Runs standalone (no `alex` package needed) in any dolfinx 0.7.x environment:
    python3 solve_plane_stress.py --micro micro_<tag>.npz --tag <tag>
"""
...
ap.add_argument('--micro', required=True, help='micro_<tag>.npz')
...
ap.add_argument('--sfun', default='1.0',
                help='transition-zone stiffness prefactor s(x); numpy expr in '
                     'x [um] (e.g. "1.0", "1.2", "1+0.3*np.exp(-((x-2000)/300)**2)")')
args = ap.parse_args()
...
sfun = eval('lambda x: ' + args.sfun, {'np': np})   # noqa: S307 (user-provided)
if rank == 0:
    print(M2.describe(cfg))

# per-cell tensors on the microstructure grid (shared with the numpy reference)
Ccell, Excell, s_map, minfo = M2.build_cell_tensors(
    euler, phase, gid, zone, (np.arange(nx) + 0.5) * step, cfg, sfun,
    verbose=(rank == 0))
...
sf = dlfx.fem.Function(Z, name='s_prefactor')
...
sf.x.array[:ncells] = s_map[cj, ci]
grainf.x.array[:ncells] = gid[cj, ci]
exf.x.array[:ncells] = Excell[cj, ci]
for f in (Cf, zonef, phasef, sf, grainf, exf):
    f.x.scatter_forward()
if rank == 0:
    print(f'grid {ny}x{nx}, {ncells} lokale Zellen, '
          f'{minfo["n_distinct_grain_tensors"]} verschiedene Korn-Tensoren')
...
res = {'tag': args.tag, 'units': 'GPa, um', 'load_axis': 'x',
       'applied_eps_xx': eps0, 'sfun': args.sfun, 'grid': [ny, nx],
       'Lx_um': Lx, 'Ly_um': Ly, 'materials': minfo}
...
# (cell values via DG0 interpolation; uniform cells -> equal weights; MPI-safe)
...
# Same cell fields as the XDMF, but reshaped onto the (ny, nx) microstructure
# grid so make_figures.py can plot them directly. Gathered onto rank 0, so it
# is correct for any number of MPI ranks.
...
if rank == 0:
    ...
    for _key in [k for k in _local if k not in ('cj', 'ci')]:
        ...
        _out[_key] = _arr
    _out['meta'] = json.dumps({'tag': args.tag, 'sfun': args.sfun,
    ...
    print(f'wrote fields_{args.tag}.npz  (Gitterfelder fuer make_figures.py)')
...
out.write_function(sf, 0.0)          # angewandter Vorfaktor s(x)
out.write_function(grainf, 0.0)      # Korn-ID -> hier ist die Kornstruktur sichtbar
out.write_function(exf, 0.0)         # E_x des Korns aus seinem eigenen Tensor
out.close()
if rank == 0:
