...
critical energy release rate \(J_c\). \added{Here, \(J\) denotes the
J-Integral as a measure of the macroscopic energetic crack-driving
force,} expressed through the criterion
\begin{equation}
  J = J_c.
...
\begin{figure}[H]
  \centering
  \resizebox{0.6\textwidth}{!}{\input{num_experiment.pdf_t}}
  \caption{Sketch of the numerical experiment introduced by Hossain
    ...
    crack-driving force. \added{The sketch illustrates the imposed near-tip
    boundary field around the resolved region and should not be read
    as a complete compact-tension specimen with locally resolved
    substructure.}}
...
with velocity \(\replaced{\dot{x}_{\rm bc}}{\dot{x}^{\rm bc}}\). The
boundary displacement field is prescribed from the asymptotic mode-I
crack-tip solution,
\begin{align}
...
\begin{equation}
  ...
  \label{eq:J-integral}
\end{equation}
\added{Here, \(\psi_{\rm el}\) denotes the degraded elastic energy density
specified in Section}~\ref{sec:phase_field_model_ductile},
\added{\(\boldsymbol{1}\) is the second-order identity tensor, and
\(\boldsymbol{\sigma}\) is the Cauchy stress tensor. In the present
diffuse-crack and heterogeneous elastoplastic setting, the boundary
integral is not used as a strictly path-independent local crack-tip
quantity in the classical LEFM sense, particularly when different
paths are compared that intersect the microstructure-resolved region
with complex crack patterns and inelastic material behavior.
Instead, following Hossain et
al.}~\cite{hossain_effective_2014}, \added{it is evaluated on the outer
boundary of the numerical experiment and interpreted as the
...
\end{equation}
\added{The norm is the Frobenius norm,
\(\|\boldsymbol{s}\|=(\boldsymbol{s}:\boldsymbol{s})^{1/2}\).}
...
\end{align}
with homogeneous Neumann boundary conditions for \(s\). \added{The mobility
\(M\) controls the relaxation rate of the phase field in the
pseudo-time evolution equation.} Quasi-static mechanical equilibrium
is imposed by
\begin{equation}
...
surrounding material. \added{Crack deflection around stiff and tough
inclusions was not observed in the present parameter range. This is
consistent with interpreting the investigated inclusion-to-matrix
property contrasts as moderate; stronger contrasts or different
offset configurations, such as those discussed by Hossain et
al.}~\cite{hossain_effective_2014}, \added{may lead to different
...
\begin{figure}[H]
  ...
  \begin{minipage}[t]{0.49\textwidth}
    ...
    \makebox[\textwidth][c]{%
      \includegraphics[
        ...
        trim=115 5 115 5,
        clip]%
      {material_layout_inclusions.png}}
  \end{minipage}
  \hfill
  \begin{minipage}[t]{0.49\textwidth}
    \centering
    ...
    \makebox[\textwidth][c]{%
      \includegraphics[
        ...
        trim=150 5 30 5,
        clip]%
      {failure_pattern_phasefield.png}}
  \end{minipage}
  \caption{Representative setup for the inclusion study. a) Full
    ...
    circular inclusions and the surrounding effective medium. \added{The
    outer boundary of the plotted domain is the contour used for the
    J-integral evaluation. The model resolves the microstructure
    inside a homogeneous embedding domain subjected to a prescribed
    near-tip field, not a complete compact-tension specimen.} b) Final
...
\added{The interfaces between inclusions and matrix are assumed perfectly
bonded; separate interface stiffnesses, strengths, or fracture
resistances are not introduced.}
...
\begin{figure}[H]
  ...
  {fig10_Jx_xct_selected_cases.png}
...
%   {fig11_Jx_sections_Jcinc_0p5.png}
...
%   \includegraphics[width=0.9\textwidth]%
%   {fig13_Jx_sections_Jcinc_1p5.png}
%   \caption{Section of the J-integral \(J_x\) versus crack-tip
...
\begin{figure}[H]
  ...
  {fig15_Jeff_wsteg_all_Jcinc.png}
...
\begin{figure}[H]
  ...
  \includegraphics[height=0.44\textheight,keepaspectratio]%
  {fig12_Jx_sections_Jcinc_1p0.png}
  \caption{Section of the J-integral \(J_x\) versus crack-tip
...
\begin{figure}[H]
  ...
  \includegraphics[width=0.95\textwidth]%
  {fig14_combined_parameter_contribution.png}
