...
try:
    ...
    parser.add_argument("--lam_matrix_param", type=float, help="Lambda parameter of the matrix")
    parser.add_argument("--mue_matrix_param", type=float, help="Mu parameter of the matrix")
    parser.add_argument("--gc_matrix_param", type=float, help="Gc parameter of the matrix")
    parser.add_argument("--sig_y_matrix_param", type=float, help="Yield stress of the matrix")
    parser.add_argument("--hard_matrix_param", type=float, help="Hardening parameter of the matrix")
    ...
    la_matrix = args.lam_matrix_param if args.lam_matrix_param is not None else args.lam_micro_param
    mu_matrix = args.mue_matrix_param if args.mue_matrix_param is not None else args.mue_micro_param
    gc_matrix = args.gc_matrix_param if args.gc_matrix_param is not None else args.gc_micro_param
    if la_matrix is None or mu_matrix is None or gc_matrix is None:
        raise ValueError("Matrix elastic and fracture parameters are required.")
    sig_y_matrix = args.sig_y_matrix_param if args.sig_y_matrix_param is not None else 1.0
    hard_matrix = args.hard_matrix_param if args.hard_matrix_param is not None else 0.2222222
    ...
    gc_matrix = 1.0
    sig_y_matrix = 1.0
    hard_matrix = 0.2222222
    la_inclusion = la_matrix
    mu_inclusion = mu_matrix
    gc_inclusion = gc_matrix
...
matrix_material_marker = 1
inclusion_material_marker = 2

effective_material_cells = mesh_tags.find(effective_material_marker)
matrix_material_cells = mesh_tags.find(matrix_material_marker)
inclusion_material_cells = mesh_tags.find(inclusion_material_marker)
...
def set_cell_function_three_materials(domain, value_effective, value_matrix, value_inclusion):
    V0 = dlfx.fem.functionspace(domain, ("DG", 0))
    field = dlfx.fem.Function(V0)
    field.x.array[effective_material_cells] = np.full_like(
        effective_material_cells, value_effective, dtype=dlfx.default_scalar_type
    )
    field.x.array[matrix_material_cells] = np.full_like(
        matrix_material_cells, value_matrix, dtype=dlfx.default_scalar_type
    )
    field.x.array[inclusion_material_cells] = np.full_like(
        inclusion_material_cells, value_inclusion, dtype=dlfx.default_scalar_type
    )
    return field

la = set_cell_function_three_materials(domain, la_effective, la_matrix, la_inclusion)
...
phaseFieldProblem = pf.StaticPhaseFieldProblem_plasticity_noll(degradationFunction=pf.cubic_degradation(0.1),
                                                   psisurf=pf.psisurf_from_function,dx=dx, sig_y=sig_y, hard=hard,alpha_n=alpha_n,e_p_n=e_p_n_3D)
...
# Effective elastic values are used only for the homogeneous surrounding region
# and for the surfing boundary-condition parameters.
...
parameters_to_write = {
        ...
        'lam_matrix_simulation': la_matrix,
        'mue_matrix_simulation': mu_matrix,
        'Gc_matrix_simulation': gc_matrix,
        'sig_y_matrix_simulation': sig_y_matrix,
        'hard_matrix_simulation': hard_matrix,
        'lam_inclusion_simulation': la_inclusion,
        'mue_inclusion_simulation': mu_inclusion,
        'Gc_inclusion_simulation': gc_inclusion,
