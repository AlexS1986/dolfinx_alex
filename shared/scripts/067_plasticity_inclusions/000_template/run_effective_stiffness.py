...
try:
    ...
    lam_matrix_param = args.lam_matrix_param if args.lam_matrix_param is not None else args.lam_micro_param
    mue_matrix_param = args.mue_matrix_param if args.mue_matrix_param is not None else args.mue_micro_param
    if lam_matrix_param is None or mue_matrix_param is None:
        raise ValueError("Matrix elastic parameters are required.")
    lam_inclusion_param = args.lam_inclusion_param if args.lam_inclusion_param is not None else lam_matrix_param
    mue_inclusion_param = args.mue_inclusion_param if args.mue_inclusion_param is not None else mue_matrix_param
except:
    print("Could not parse arguments")
    lam_matrix_param = 1.0
    mue_matrix_param = 1.0
    lam_inclusion_param = 1.0
    mue_inclusion_param = 1.0
...
lam = het.set_cell_function_heterogeneous_material(
    domain, lam_inclusion_param, lam_matrix_param, inclusion_cells, matrix_cells
)
mu = het.set_cell_function_heterogeneous_material(
    domain, mue_inclusion_param, mue_matrix_param, inclusion_cells, matrix_cells
)
...
def after_last_timestep():
    ...
    if rank == 0:
        ...
        parameters_to_write = {
                ...
                "lam_matrix_hom_input": lam_matrix_param,
                "mue_matrix_hom_input": mue_matrix_param,
                "lam_inclusion_hom_input": lam_inclusion_param,
                "mue_inclusion_hom_input": mue_inclusion_param,
        }
