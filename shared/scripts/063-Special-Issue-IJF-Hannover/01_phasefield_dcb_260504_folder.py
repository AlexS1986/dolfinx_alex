...
def parse_args(argv, rank=0):
    ...
    if rank == 0:
        ...
        print(f"[INFO] Dataset start: {ds_start}, end: {ds_end}, case: {case}, split: {split}")

    return folder, ds_start, ds_end, case, split

folder_path, dataset_start, dataset_end, case_param, split_param = parse_args(sys.argv)
...
for dataset_index, dataset_folder in dataset_specs:
    ...
    convergence_log_path = os.path.join(folder_path, f"convergence_log_{split_param}.txt")
    if rank == 0:
...
# --------------------------------------------------
    ...
    def read_E_average_from_vol_json(x_val):
        vol_filename = os.path.join(folder_path, f"vol_{dataset_label}_vary_{split_name}.json")
    ...
    for split_name, case in [
        ...
        results_xdmf_path = os.path.join(folder_path, f"results_{dataset_label}_{case}_{split_name}.xdmf")
        outputfile_graph_path = os.path.join(folder_path, f"result_graphs_{dataset_label}_{case}_{split_name}.txt")
       ...
       #λ_arc_length = dlfx.fem.Constant(domain, petsc.ScalarType(0.0000000001))
        ...
        logfile_path = alex.os.logfile_full_path(folder_path, f"{script_name_without_extension}_{dataset_label}_{case}_{split_name}")
        ...
        def write_vol_data_to_file():
            if rank == 0:
                vol_path = os.path.join(folder_path, f"vol_{dataset_label}_{case}_{split_name}.json")
                volumes_data = {
                        "vol": vol,
                        "E_average": E_average,
