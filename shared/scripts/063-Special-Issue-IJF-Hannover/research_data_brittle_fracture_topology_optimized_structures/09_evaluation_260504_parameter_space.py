...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional beta_phi dataset supplying shared min/max cases when the "
            "varying-porosity cases use a different fixed beta."
        ),
    )
    parser.add_argument(
        "--splits",
...
def label_for_record(record: ResultRecord, include_beta: bool = True) -> str:
    ...
    parts.extend([
        ...
        rf"$\rho={record.rho:g}$",
        length_label(r"\beta_s", record.epsilon),
    ])
    return ", ".join(parts)
...
def beta_phi_legend_label(value: float) -> str:
    return rf"$\beta_{{\phi}}={value:g}\,\mathrm{{mm}}^2$"
...
def line_style_for_beta(beta: float):
    for known_beta, line_style in BETA_LINESTYLES.items():
        if math.isclose(beta, known_beta):
            return line_style
    return "-"
...
def add_beta_rho_legends(fig, beta_values: tuple[float, ...], rho_values: list[float]) -> None:
    beta_handles = [
        Line2D(
            [0],
            [0],
            color="#626262",
            linestyle=line_style_for_beta(beta),
            linewidth=2.4,
            label=beta_phi_legend_label(beta),
        )
    ...
    beta_legend = fig.legend(
        beta_handles,
        [handle.get_label() for handle in beta_handles],
        loc="upper left",
    ...
    style_legend(beta_legend)
    rho_legend = fig.legend(
        rho_handles,
        [handle.get_label() for handle in rho_handles],
        loc="upper right",
...
def select_beta_with_shared_constant_cases(
    ...
    selected = filter_by_beta(records, beta)
    if shared_constant_beta is not None and not math.isclose(beta, shared_constant_beta):
        selected.extend(
            record
            for record in records
            if math.isclose(record.beta, shared_constant_beta)
            and record.case in {"min", "max"}
        )
...
def plot_beta_comparison_reaction_curve(
    records: list[ResultRecord],
    output_folder: Path,
    beta_values: tuple[float, ...] = (0.001, 0.01, 0.05),
    split: str = "spectral",
    a_value: int = 6,
    epsilon: float = 0.03,
) -> None:
    """Plot the Fig. 17(a) response comparison for the varying-porosity cases."""
    subset = [
        record
        for record in records
        if record.split == split
        and record.a == a_value
        and math.isclose(record.epsilon, epsilon)
        and record.case == "vary"
        and any(math.isclose(record.beta, beta) for beta in beta_values)
    ]
    if not all(any(math.isclose(record.beta, beta) for record in subset) for beta in beta_values):
        return
    ...
    for rho in sorted({record.rho for record in subset}):
        for beta in beta_values:
            beta_records = [
                ...
                if math.isclose(record.rho, rho) and math.isclose(record.beta, beta)
            ]
            if not beta_records:
                continue
            record = beta_records[0]
            data = load_graph_data(record.path)
            displacement = np.abs(data[:, 1])
            reaction = np.abs(data[:, QUANTITIES["Ry"]["column"]])
            peak_index = peak_reaction_index(data)
            ...
            ax.plot(
                displacement,
                reaction,
                label=rf"$\rho={rho:g}$, $\beta_\phi={beta:g}$",
                color=color_for_rho(rho),
                linestyle=line_style_for_beta(beta),
                linewidth=1.8,
            )
    ...
    ax.set_xlabel(r"$u_y$ in mm")
    ax.set_ylabel(quantity_label("Ry", "curve_label"))
    ax.set_xlim(0.0, uy_display_maximum(None))
    ax.set_ylim(0.0, nice_axis_upper_limit(max_force))
    ax.grid(True, alpha=0.3)
    add_beta_rho_legends(fig, beta_values, sorted({record.rho for record in subset}))
    format_axes(ax)
    fig.tight_layout(rect=(0.0, 0.0, 1.0, 0.62))
    fig.savefig(
        split_output_folder(output_folder, split)
        / f"Beta_comparison_Ry_vs_uy_{split}_a_{a_value}_eps{float_filename_token(epsilon)}.png",
        dpi=300,
    )
    plt.close(fig)


def plot_beta_comparison_peak_reaction_vs_epsilon(
    records: list[ResultRecord],
    output_folder: Path,
    beta_values: tuple[float, ...] = (0.001, 0.01, 0.05),
    split: str = "spectral",
    ...
    """Plot the Fig. 18(a) peak-force comparison for the varying-porosity cases."""
    subset = [
        record
        for record in records
        if record.split == split
    ...
    for rho in sorted({record.rho for record in subset}):
        for beta in beta_values:
            ...
            if not beta_records:
                continue
            values = [record.max_values["Ry"] for record in beta_records]
            max_force = max(max_force, max(values))
            ax.plot(
                [record.epsilon for record in beta_records],
                values,
                color=color_for_rho(rho),
                linestyle=line_style_for_beta(beta),
                marker="o",
    ...
    ax.set_xlabel(length_axis_label(r"\beta_s"))
    ax.set_ylabel(quantity_label("Ry"))
    ...
    fig.savefig(
        split_output_folder(output_folder, split)
        / f"Beta_comparison_max_Ry_vs_epsilon_{split}_a_{a_value}.png",
        dpi=300,
    )
    plt.close(fig)
        ...
        for case in sorted({record.case for record in split_records}, key=case_order):
            ...
            for ax, a_value in zip(axes.ravel(), a_values):
                ...
                for rho, epsilon in sorted({(record.rho, record.epsilon) for record in subset}):
                    ...
                    label = rf"$\rho={rho:g}$, " + length_label(r"\beta_s", epsilon)
                    ax.plot(
        ...
        for case in sorted({record.case for record in split_records}, key=case_order):
            ...
            for ax, a_value in zip(axes.ravel(), a_values):
                ...
                for epsilon in sorted({record.epsilon for record in subset}):
                    ...
                    label = length_label(r"\beta_s", epsilon)
...
def plot_peak_metrics_vs_epsilon_grid(
    ...
    for split in sorted({record.split for record in records}):
        ...
        for a_value in sorted({record.a for record in split_records}):
            ...
            ax_elastic.set_xlabel(length_axis_label(r"\beta_s"))
            ax_fracture.set_xlabel(length_axis_label(r"\beta_s"))
        ...
        for case in sorted({record.case for record in split_records}, key=case_order):
            ...
            for epsilon in epsilon_values:
                ...
                ax.plot(
                    ...
                    linewidth=1.5,
                    alpha=0.65,
                    label=case_label(case) + ", " + length_label(r"\beta_s", epsilon),
                )
...
def main() -> None:
    ...
    fixed_beta_records = select_beta_with_shared_constant_cases(
        records,
        fixed_beta,
    ...
    plot_beta_comparison_reaction_curve(records, output_folder)
    plot_beta_comparison_peak_reaction_vs_epsilon(records, output_folder)
